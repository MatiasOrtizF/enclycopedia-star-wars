        ...
        <>
            {datas.map((data , index) => (
                <View style={styles.box} key={index}>
                    ...
                    <Text style={styles.genderHome}>{data.classification}</Text>
                    <View style={{flexDirection:"row" , justifyContent:"space-between"}}>
                        <Text style={styles.heightHome}>{data.language}</Text>
                        <TouchableOpacity>
                            <Text onPress={()=>navigation.navigate('SpecieScreen', {specieInfo: data.url})} style={{color:"orange" , fontWeight:600 , fontSize:15}}>More Info</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            ))}
