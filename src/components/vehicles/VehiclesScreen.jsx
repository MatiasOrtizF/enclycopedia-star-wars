            ...
            {datas.map((data , index) => (
                <View style={styles.box} key={index}>
                    ...
                    <View style={{flexDirection:"row" , justifyContent:"space-between"}}>
                        <Text style={styles.heightHome}>{data.vehicle_class}</Text>
                        <TouchableOpacity>
                            <Text onPress={()=>navigation.navigate('VehicleScreen', {vehicleInfo: data.url})} style={{color:"orange" , fontWeight:600 , fontSize:15}}>More Info</Text>
                        </TouchableOpacity>
                    </View>
                </View>
            ))}
